...
async function perform() {
  ...
  try {
    const [expired_files] = await db.query<[File[]]>(
      surql`SELECT * FROM file WHERE expires_at < time::now()-5m;`,
    );
    ...
    last_success = true;
    logger.info(
      `File cleanup complete: count = ${deleted_count} & duration = ${last_duration}`,
    );
  } catch (err) {
