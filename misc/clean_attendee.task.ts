...
async function perform() {
  ...
  try {
    const [expired_attendees] = await db.query<[Attendee[]]>(
      surql`SELECT * FROM attendee WHERE expires_at < time::now()-5m;`,
    );
    ...
    last_success = true;
    logger.info(
      `Attendee cleanup complete: count = ${deleted_count} & duration = ${last_duration}`,
    );
  } catch (err) {
