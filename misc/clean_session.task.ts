...
async function perform() {
  ...
  try {
    const [expired_sessions] = await db.query<[Session[]]>(
      surql`SELECT * FROM session WHERE expires_at < time::now()-5m;`,
    );
    ...
    last_success = true;
    logger.info(
      `Session cleanup complete: count = ${deleted_count} & duration = ${last_duration}`,
    );
  } catch (err) {
