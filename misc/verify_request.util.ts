...
export function verify_request(roles: Array<"organiser" | "attendee">) {
  return async (c: Context, next: Next) => {
    ...
    const bearer = c.req.header("Authorization")?.split(" ")[1] ||
      getCookie(c, "visitor_token") || getCookie(c, "access_token");
    ...
    if (
      unixTimestamp > payload.exp || payload.iat > unixTimestamp ||
      roles.indexOf(payload.role) < 0
    ) {
      throw new HTTPException(401, { message: "Access denied" });
    }

    if (!payload.role || !payload.sid || !payload.id) {
      throw new HTTPException(401, { message: "Badly formatted access token" });
    }

    if (!roles.find((val) => val === payload.role)) {
      throw new HTTPException(403, { message: "You are not authorised" });
    }
    ...
    if (payload.role === "organiser") {
      ...
      const [session] = await db.query<[Session]>(
        surql`SELECT * FROM ONLY session WHERE id = ${new RecordId(
          "session",
          payload.sid,
        )} AND user = ${new RecordId(payload.role, payload.id)} LIMIT 1;`,
      );
      if (!session) {
        throw new HTTPException(401, { message: "Session not found" });
      }
      if (Date.now() > new Date(session.expires_at).valueOf()) {
