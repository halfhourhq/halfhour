...
async function perform() {
  ...
  try {
    const [expired_organisers] = await db.query<[Organiser[]]>(
      surql`SELECT * FROM organiser WHERE end_time < time::now()-5m;`,
    );
    ...
    last_success = true;
    logger.info(
      `Organiser cleanup complete: count = ${deleted_count} & duration = ${last_duration}`,
    );
  } catch (err) {
