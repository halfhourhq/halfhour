...
storage.get(
  "/connection/:id",
  verify_request(["attendee", "organiser"]),
  async (c) => {
    ...
    switch (user.table) {
      case "organiser":
        query =
          surql`SELECT * FROM ONLY connects_with WHERE id = ${new RecordId(
            "connects_with",
            connection_id,
          )} AND in = ${new RecordId("organiser", user.id)} LIMIT 1;`;
        break;
      case "attendee":
        query =
          surql`SELECT * FROM ONLY connects_with WHERE id = ${new RecordId(
            "connects_with",
            connection_id,
          )} AND out = ${new RecordId("attendee", user.id)} LIMIT 1;`;
        break;
    }
    ...
    const [organiser] = await db.query<[Organiser]>(
      surql`SELECT * FROM ONLY organiser WHERE id = ${connection.in} LIMIT 1;`,
    );

    if (organiser.start_time > new Date()) {
      throw new HTTPException(403, {
        message: "The meeting is not active yet",
      });
    }
    if (organiser.end_time < new Date()) {
      throw new HTTPException(403, {
        message: "The meeting is not longer active",
      });
    }

    const [file, opened] = await db.query<[File, number]>(surql`
    SELECT * FROM ONLY file WHERE id = ${new RecordId(
      "file",
      file_id,
    )} AND connection = ${connection.id} LIMIT 1;
    RETURN count(SELECT * FROM opened_by WHERE in = ${new RecordId(
      "file",
      file_id,
    )} AND out = ${new RecordId(user.table, user.id)});
  `);
    if (!file) throw new HTTPException(400, { message: "File not found" });
    if (opened > 0) {
      throw new HTTPException(400, {
        message: "You have already opened this file",
      });
    }
    if (file.downloads_count >= file.downloads_total) {
      throw new HTTPException(400, {
        message: "You have reached the maximum downloads on this file",
      });
    }

    const b2_auth_res = await fetch(
      endpoint + "/b2api/v4/b2_authorize_account",
      {
        method: "GET",
        headers: {
    ...
    if (!b2_auth_res.ok) {
      throw new HTTPException(404, {
        message: b2_auth.message,
      });
    }

    const b2_download_res = await fetch(
      `${endpoint}/b2api/v4/b2_download_file_by_id?fileId=${file.b2_file_id}`,
      {
        method: "GET",
        headers: {
    ...
    await db.query(
      surql`RELATE ${file.id}->opened_by->${new RecordId(
        user.table,
        user.id,
      )};`,
    );
    ...
    c.header(
      "Content-Type",
      b2_download_res.headers.get("Content-Type") || file.content_type,
    );
...
storage.post(
  ...
  async (c) => {
    ...
    if (total_storage >= storage_limit) {
      throw new HTTPException(400, {
        message: "This meeting has reached its storage limit",
      });
    }
    ...
    const schema = z.object({
      ...
      size: z.number({ message: "Provide the file size" }).min(
        1,
        "The file size must be at least 1 byte",
      ).max(5 * 1024 * 2024, "The file size cannot exceed 5 MB"),
      file: z.instanceof(ArrayBuffer, { message: "Provide the file" }).refine(
        (arr) => arr.byteLength <= 5 * 1024 * 2024 + 16,
    ...
    if (validation.success === false) {
      ...
      if (formatted.sha1) {
        formatted.sha1?._errors.reverse().forEach((val) => message = `${val}`);
      }
      if (formatted.type) {
        formatted.type?._errors.reverse().forEach((val) => message = `${val}`);
      }
      if (formatted.name) {
        formatted.name?._errors.reverse().forEach((val) => message = `${val}`);
      }
      if (formatted.size) {
        formatted.size?._errors.reverse().forEach((val) => message = `${val}`);
      }
      if (formatted.file) {
        formatted.file?._errors.reverse().forEach((val) => message = `${val}`);
      }
    ...
    const b2_upload_res = await fetch(
      `${endpoint}/b2api/v4/b2_get_upload_url?bucketId=${bucket}`,
      {
        method: "GET",
        headers: {
          "Authorization": b2_auth.authorizationToken,
    ...
    const file_storage_path = `${
      organiser.meeting_tag.replace(/[^A-Z]/g, "")
    }/${sanitisation.storage_name}`;
    ...
    const [new_file] = await db.query<[File]>(
      surql`CREATE ONLY file CONTENT ${file_content};`,
    );
