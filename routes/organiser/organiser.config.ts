...
export const organiser_table = {
  ...
  fields: {
    name:
      `DEFINE FIELD name ON TABLE organiser TYPE string DEFAULT 'Sailor' READONLY;`,
    ...
    created_at:
      "DEFINE FIELD created_at ON TABLE organiser TYPE datetime DEFAULT time::now() READONLY;",
    updated_at:
      "DEFINE FIELD updated_at ON TABLE organiser TYPE datetime DEFAULT time::now();",
  },
  indices: {
    unique_meeting_tag:
      "DEFINE INDEX unique_meeting_tag ON TABLE organiser FIELDS meeting_tag UNIQUE;",
  },
};
