...
organiser.get("/", verify_request(["organiser"]), async (c) => {
  ...
  const [organiser] = await db.query<[PartialOrganiser]>(surql`
    ...
    created_at,
    name
    FROM ONLY organiser WHERE id = ${new RecordId(
    "organiser",
    user.id,
  )} AND end_time >= time::now() LIMIT 1;
  `);

  if (!organiser) {
    throw new HTTPException(404, { message: "The meeting is not found" });
  }

  const connection = (await db.query<[Connection[]]>(surql`
    SELECT id as connection_id, out.id as attendee_id, out.response_tag as response_tag, out.name as attendee_name FROM connects_with WHERE in.id = ${new RecordId(
    "organiser",
    user.id,
  )};
  `))[0][0];
...
organiser.post("/", rate_limiter, async (c) => {
  const schema = z.object({
    start_time: z.preprocess(
      ...
      return val > new Date();
    }, { message: "You cannot schedule a meeting to a past date" }).refine(
      (val) => val <= new Date(Date.now() + 1000 * 60 * 60 * 24 * 7),
      {
        message:
          "You cannot schedule a meeting more than 7 days into the future",
      },
    ),
  ...
  if (validation.success === false) {
    ...
    if (formatted.start_time) {
      formatted.start_time?._errors.reverse().forEach((val) =>
        message = `${val}`
      );
    }
  ...
  const [new_organiser] = await db.query<[Organiser]>(
    surql`CREATE ONLY organiser CONTENT ${organiser_content};`,
  );
  return c.json({
    id: new_organiser.id,
...
organiser.post("/auth/start", rate_limiter, async (c) => {
  ...
  const [organiser] = await db.query<[Organiser]>(
    surql`SELECT * FROM ONLY organiser WHERE meeting_tag = ${meeting_tag} LIMIT 1;`,
  );
  if (!organiser) {
    throw new HTTPException(400, { message: "Organiser was not found" });
  }
...
organiser.post("/auth/finish", rate_limiter, async (c) => {
  ...
  const [session] = await db.query<[number]>(
    surql`RETURN count(SELECT * FROM session WHERE user = ${organiser.id});`,
  );
  ...
  if (match === false) {
    throw new HTTPException(400, {
      message: "The meeting tag or the password is wrong",
    });
  }

  const [connection] = await db.query<[ConnectsWith]>(
    surql`SELECT * FROM ONLY connects_with WHERE in = ${organiser.id} LIMIT 1;`,
  );
