...
export const attendee_table = {
  ...
  fields: {
    ...
    created_at:
      "DEFINE FIELD created_at ON TABLE attendee TYPE datetime DEFAULT time::now() READONLY;",
    updated_at:
      "DEFINE FIELD updated_at ON TABLE attendee TYPE datetime DEFAULT time::now();",
  },
  indices: {
    unique_response_tag:
      "DEFINE INDEX unique_response_tag ON TABLE attendee FIELDS response_tag UNIQUE;",
  },
};

export const requests_to_table = {
  table:
    "DEFINE TABLE requests_to SCHEMAFULL TYPE RELATION IN attendee OUT organiser;",
  fields: {
    created_at:
      "DEFINE FIELD created_at ON TABLE requests_to TYPE datetime DEFAULT time::now() READONLY;",
  },
  indices: {
    unique_in_out:
      "DEFINE INDEX unique_in_out ON TABLE requests_to FIELDS in, out UNIQUE;",
    unique_in: "DEFINE INDEX unique_in ON TABLE requests_to FIELDS in UNIQUE;",
    unique_out:
      "DEFINE INDEX unique_out ON TABLE requests_to FIELDS out UNIQUE;",
  },
};
