...
attendee.get("/", verify_request(["attendee"]), async (c) => {
  ...
  const [request] = await db.query<[RequestsTo]>(
    surql`SELECT * FROM ONLY requests_to WHERE in = ${new RecordId(
      "attendee",
      user.id,
    )} LIMIT 1;`,
  );
  if (!request) throw new HTTPException(404, { message: "Attendee not valid" });
  const connection = (await db.query<[ConnectsWith[]]>(
    surql`SELECT * FROM connects_with WHERE out = ${new RecordId(
      "attendee",
      user.id,
    )};`,
  ))[0][0];
...
attendee.post("/", rate_limiter, async (c) => {
  const schema = z.object({
    ...
    }, { message: "Name cannot be longer than 20 characters" }).refine(
      (val) => {
  ...
  if (validation.success === false) {
    ...
    if (formatted.meeting_tag) {
      formatted.meeting_tag._errors.reverse().forEach((val) =>
        message = `${val}`
      );
    }
    if (formatted.password_hash) {
      formatted.password_hash?._errors.reverse().forEach((val) =>
        message = `${val}`
      );
    }
    if (formatted.password_salt) {
      formatted.password_salt?._errors.reverse().forEach((val) =>
        message = `${val}`
      );
    }
    if (formatted.public_key) {
      formatted.public_key?._errors.reverse().forEach((val) =>
        message = `${val}`
      );
    }
    if (formatted.keypair_salt) {
      formatted.keypair_salt?._errors.reverse().forEach((val) =>
        message = `${val}`
      );
    }
    if (formatted.name) {
      formatted.name?._errors.reverse().forEach((val) => message = `${val}`);
    }
  ...
  const organiser = (await db.query<[Organiser[]]>(
    surql`SELECT * FROM organiser WHERE meeting_tag = ${meeting_tag} LIMIT 1;`,
  ))[0][0];
  if (!organiser) {
    throw new HTTPException(400, { message: "Action not allowed" });
  }
  ...
  const [new_attendee] = await db.query<[Attendee]>(
    surql`CREATE ONLY attendee CONTENT ${attendee_content};`,
  );
  await db.query<[RequestsTo[]]>(
    surql`RELATE ${new_attendee.id}->requests_to->${organiser.id};`,
  );
...
attendee.post("/auth/start", rate_limiter, async (c) => {
  ...
  if (validation.success === false) {
    ...
    let message: string = "";
    formatted._errors.reverse().forEach((val) => message = `${val}`);
    if (formatted.response_tag) {
      formatted.response_tag?._errors.reverse().forEach((val) =>
        message = `${val}`
      );
    }
    throw new HTTPException(404, { message: message });
  ...
  const [attendee] = await db.query<[Attendee]>(
    surql`SELECT * FROM ONLY attendee WHERE response_tag = ${response_tag} LIMIT 1;`,
  );
  if (!attendee) {
    throw new HTTPException(400, { message: "Attendee was not found" });
  }

  const [organiser] = await db.query<[Organiser]>(
    surql`SELECT VALUE out.* FROM ONLY requests_to WHERE in = ${attendee.id} LIMIT 1;`,
  );
  if (!organiser) {
    throw new HTTPException(400, { message: "Meeting was not found" });
  }
...
attendee.post("/auth/finish", rate_limiter, async (c) => {
  ...
  const validation = schema.safeParse({
    password_hash: body.password_hash,
    response_tag: body.response_tag,
  });
  ...
  const session = (await db.query<[number]>(
    surql`RETURN count(SELECT * FROM session WHERE user = ${attendee.id});`,
  ))[0];

  if (session > 0) {
    throw new HTTPException(400, {
      message:
        "You have an active session elsewhere, logout first before proceeding",
    });
  }
  ...
  if (match === false) {
    throw new HTTPException(400, {
      message: "The response tag or the password is wrong",
    });
  }

  const connection = (await db.query<[ConnectsWith[]]>(
    surql`SELECT * FROM connects_with WHERE in = ${organiser.id} AND out = ${attendee.id};`,
  ))[0][0];

  const attendee_id = attendee.id.id.toString();
  const jwt_secret = Deno.env.get("JWT_SECRET");
  const jwt_expiration = new Date(organiser.end_time);
  if (!jwt_expiration || !jwt_secret) {
    throw new HTTPException(400, {
  ...
  const [new_session] = await db.query<[Session]>(
    surql`CREATE ONLY session CONTENT ${session_content};`,
  );

  const token = await new SignJWT({
    id: attendee_id,
    sid: new_session.id.id.toString(),
    role: "attendee",
  }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime(
    jwt_expiration,
  ).sign(encodedSecret);
