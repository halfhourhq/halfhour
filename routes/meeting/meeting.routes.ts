...
meeting.post("/connect", verify_request(["organiser"]), async (c) => {
  ...
  const organiser = await db.select<Organiser>(
    new RecordId("organiser", user.id),
  );
  if (!organiser) {
    throw new HTTPException(400, { message: "Meeting not found" });
  }
  ...
  if (!attendee) {
    throw new HTTPException(400, {
      message: "The attendee response does not exist",
    });
  }

  const has_requested = (await db.query<Array<number>>(
    surql`RETURN count(SELECT * FROM requests_to WHERE in = ${attendee.id} AND out = ${organiser.id});`,
  ))[0];
  if (has_requested === 0) {
    throw new HTTPException(400, { message: "Forbidden is connection" });
  }

  const [connects] = await db.query<[number]>(
    surql`RETURN count(SELECT * FROM connects_with WHERE in = ${organiser.id});`,
  );
  if (connects > 0) {
    throw new HTTPException(400, {
      message: "Only one connected attendee per meeting is allowed",
    });
  }
...
meeting.get(
  "/connection/:id",
  verify_request(["organiser", "attendee"]),
  async (c) => {
    const user = c.get("user");
    ...
    const text = (await db.query<Array<ConversationWith[]>>(
      surql`SELECT * FROM conversation_with WHERE out = ${connection.id} ORDER created_at ASC LIMIT 100;`,
    ))[0];
...
meeting.get(
  ...
  async (c, next) => {
    ...
    if (!attendee || !organiser) {
      throw new HTTPException(403, {
        message: "The connection is not allowed",
      });
    }

    if (organiser.start_time > new Date()) {
      throw new HTTPException(403, {
        message: "The connection is not active yet",
      });
    }
    if (organiser.end_time < new Date()) {
      throw new HTTPException(403, {
        message: "The connection is not longer active",
      });
    }
    await next();
  },
  upgradeWebSocket(async (c) => {
    const user: {
      id: string;
      table: "organiser" | "attendee";
      session: string;
    } = c.get("user");

    const user_id = new RecordId(user.table, user.id);

    const meet = await db.select<ConnectsWith>(
      new RecordId("connects_with", c.req.param("id")),
    );

    const conversation = conversations.get(meet.id.toString()) ??
      new Map<string, WSContext<WebSocket>>();
    const messages = all_messages.get(meet.id.toString()) ??
      new Set<ConversationWith>();
          ...
          switch (msg.type) {
            case "text": {
              ...
              if (msg.data.file) {
                ...
                const storage = await db.query<[number]>(surql`
