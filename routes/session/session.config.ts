...
export const session_table = {
  ...
  fields: {
    ...
    created_at:
      `DEFINE FIELD created_at ON TABLE session TYPE datetime DEFAULT time::now() READONLY;`,
    expires_at: `DEFINE FIELD expires_at ON TABLE session TYPE datetime;`,
  },
  indices: {
    unique_user:
      "DEFINE INDEX unique_user ON TABLE session FIELDS user UNIQUE;",
  },
};
...
export const login_table = {
  ...
  fields: {
    ...
    created_at:
      `DEFINE FIELD created_at ON TABLE login TYPE datetime DEFAULT time::now() READONLY;`,
