...
async function getSurreal(): Promise<Surreal> {
  ...
  if (!url || !user || !pass || !namespace || !database) {
    ...
    throw new Error(
      "Database connection not established: DB_URL, DB_USER, DB_PASS, DB_NS & DB_DB variables must be provided.",
    );
  }
  ...
  try {
    ...
    console.log(
      "Database successfully connected to " + namespace + " namespace and " +
        database + " database.",
    );
    return connection;
  } catch (error) {
