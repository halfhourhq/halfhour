...
import {
  attendee_table,
  requests_to_table,
} from "../routes/attendee/attendee.config.ts";
import {
  connects_with_table,
  conversation_with_table,
} from "../routes/meeting/meeting.config.ts";
import {
  login_table,
  session_table,
} from "../routes/session/session.config.ts";
import {
  file_table,
  opened_by_table,
} from "../routes/storage/storage.config.ts";
...
export async function startup() {
  try {
    ...
    if (tables.indexOf("organiser") < 0) {
      ...
      const present_fields = Object.entries(organiser_info[0].fields).map(
        (field) => field[0],
      );
      ...
      const present_indexes = Object.entries(organiser_info[0].indexes).map(
        (field) => field[0],
      );
    ...
    if (tables.indexOf("attendee") < 0) {
      ...
      const present_fields = Object.entries(attendee_info[0].fields).map(
        (field) => field[0],
      );
      ...
      const present_indexes = Object.entries(attendee_info[0].indexes).map(
        (field) => field[0],
      );
    ...
    if (tables.indexOf("requests_to") < 0) {
      ...
      const requestsTo_info = await db.query<any[]>(
        "INFO FOR TABLE requests_to;",
      );

      const present_fields = Object.entries(requestsTo_info[0].fields).map(
        (field) => field[0],
      );
      ...
      const present_indexes = Object.entries(requestsTo_info[0].indexes).map(
        (field) => field[0],
      );
    ...
    if (tables.indexOf("connects_with") < 0) {
      ...
      const connectsWith_info = await db.query<any[]>(
        "INFO FOR TABLE connects_with;",
      );

      const present_fields = Object.entries(connectsWith_info[0].fields).map(
        (field) => field[0],
      );
      ...
      const present_indexes = Object.entries(connectsWith_info[0].indexes).map(
        (field) => field[0],
      );
    ...
    if (tables.indexOf("conversation_with") < 0) {
      ...
      const conversationWith_info = await db.query<any[]>(
        "INFO FOR TABLE conversation_with;",
      );
      const present_fields = Object.entries(conversationWith_info[0].fields)
        .map((field) => field[0]);
    ...
    if (tables.indexOf("session") < 0) {
      ...
      const present_fields = Object.entries(session_info[0].fields).map(
        (field) => field[0],
      );
      ...
      const present_indexes = Object.entries(session_info[0].indexes).map(
        (field) => field[0],
      );
    ...
    if (tables.indexOf("login") < 0) {
      ...
      const present_fields = Object.entries(login_info[0].fields).map((field) =>
        field[0]
      );
      ...
      const present_indexes = Object.entries(login_info[0].indexes).map(
        (field) => field[0],
      );
    ...
    if (tables.indexOf("file") < 0) {
      ...
      const present_fields = Object.entries(file_info[0].fields).map((field) =>
        field[0]
      );
      ...
      const present_indexes = Object.entries(file_info[0].indexes).map(
        (field) => field[0],
      );
    ...
    if (tables.indexOf("opened_by") < 0) {
      ...
      const present_fields = Object.entries(opened_by_info[0].fields).map(
        (field) => field[0],
      );
      ...
      const present_indexes = Object.entries(opened_by_info[0].indexes).map(
        (field) => field[0],
      );
